...
# then filter to only retain deer and puma and then get project n
dets_samp <- dets %>% filter(Species == "Deer" | Species == "Puma")
...
for(i in 1:length(deps)){
  ...
  # any sp1 detected?
  if(any(dets$Species[ind] == 'Deer')){ # sp1 is the subordinate species
    
    # indices of detections
    sp1_ind[[i]] <- which(dets$Species[ind] == 'Deer')
